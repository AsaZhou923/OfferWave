...
除“登录注册”和“公开职位列表”接口外，所有接口均需在 HTTP Header 中携带 Token。
...
系统采用标准的 JSON 格式响应，包含状态码、消息提示和业务数据。
...
| **参数名**   | **类型** | **必填** | **说明**                               |
|--------------|----------|----------|----------------------------------------|
| **username** | string   | **是**   | 登录账号 (4-50位，可以是手机号/邮箱/自定义字母) |
...
- **400 Bad Request**: 
  ...
  - `{"code": 400, "message": "用户名长度必须在4到50个字符之间"}`
  - `{"code": 400, "message": "密码长度至少为6位"}`
...
```json
{
  ...
  "data": {
    ...
    "preferences": {
      "industry": ["互联网", "金融"], // 解析自 pref_industry (JSON/CSV)
      "city": "北京",
      "job": "后端开发"
    },
    "membership": {
      ...
      "privileges": {      // 解析自 memberships.privileges
        "max_track_count": 10,
    ...
    "stats": {
      "tracked_count": 8,   // 当前已追踪/收藏的职位数 (用于前端判断是否超限)
...
| 参数名        | 类型   | 必填 | 说明                          |
| :------------ | :----- | :--- | :---------------------------- |
| pref_industry | string | 否   | "偏好行业 (如 "互联网,教育")" |
| pref_city     | string | 否   | "偏好城市 (如 "上海")"        |
| pref_job      | string | 否   | "偏好岗位 (如 "Java开发")"    |
...
| 参数名                  | 类型 | 必填 | 说明                        |
| :---------------------- | :--- | :--- | :-------------------------- |
| target_level_id         | int  | 是   | 目标等级 ID (如 2 代表 VIP) |
| **成功响应 (HTTP 200)** |      |      |                             |
...
此模块提供职位的检索、筛选和详情展示。
**注意**：此模块接口**无需鉴权**即可访问，但如果 Request Header 中带了 Token，后端会在返回详情时附加“我的投递状态”。
...
| **keyword**      | string | 否   | 搜索关键词 (匹配公司名或岗位名)                     |
    ...
    "records": [
      {
        ...
        "deadline": "2026-03-31",  // 前端需比较此时间：若 < 7天 则显示红色
        "process_stage": "网申开启",
        "updated_at": "2026-02-18 10:00:00",
        "is_urgent": true          // 后端计算字段: true表示快截止
      },
      {
        "id": 2049,
        "company_name": "腾讯",
...
```json
{
  ...
  "data": {
    ...
    "job_title": "后端开发工程师",
    "announcement": "职位描述...", 
    "apply_link": "[https://job.bytedance.com/](https://job.bytedance.com/)...", // 投递跳转链接
    "test_info": "需进行在线测评",
    "deadline": "2026-03-31",
    ...
    "my_status": {
      "is_collected": true,
      "delivery_status": 1, // 1:已投递
      "delivery_status_str": "已投递",
      "user_note": "面试定在周三"
    }
  }
}
...
- **逻辑约束**:
  1. 检查用户会员等级。
  2. 如果是**普通用户**且 `is_collected=true` 或 `delivery_status > 0` 的总数已达到权益上限 (如 10 个)，则拒绝操作。
...
{
  "code": 403,
  "message": "普通用户只能追踪 10 个职位，请升级 VIP 解锁无限权益",
  "data": {
    "current_count": 10,
    "limit": 10,
...
**鉴权**: 使用专用的 API Key (在 Header 中传递)，不使用用户 Token。
...
- **401 Unauthorized**: `{"code": 401, "message": "无效的 API Key"}`
